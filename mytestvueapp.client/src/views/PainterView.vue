  ...
  <Toolbar class="fixed bottom-0 left-0 right-0 m-2">
    <template #start>
      <div class="flex gap-2">
        ...
        <SaveImageToFile :art="art"
                         :fps="fps"
                         :filtered="false"
                         :filtered-art="''"
                         :gif-from-viewer="['']"></SaveImageToFile>
    ...
    <template #center>
      ...
      <ColorSelection v-model:color="art.pixelGrid.backgroundColor"
                      ...
                      @enable-key-binds="keyBindActive = true"
                      @disable-key-binds="keyBindActive = false" />
      <FrameSelection v-if="art.pixelGrid.isGif"
...
<script setup lang="ts">
  ...
  async function saveGifFromImage() {
    GIFCreationService.createGIF(props.gifFromViewer!, fps.value);
  }

  // function saveFilteredImage() {
  //   const canvas = document.createElement("canvas");
  //   canvas.width = art.value.pixelGrid.width;
  //   canvas.height = art.value.pixelGrid.width;
  ...
  //     art.value.pixelGrid.width
  //   );
  //   const numPixels = art.value.pixelGrid.width * art.value.pixelGrid.width;
